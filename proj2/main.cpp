...
int main(int argc, char *argv[]) {
  ...
  cxxopts::Options options("AVS: Marching cubes",
                           "AVS Assignment 2 - Parallel Marching Cubes");
  options.add_options()("i,input", "Input field file",
                        cxxopts::value<std::string>())(
      "o,output", "Output mesh file",
      cxxopts::value<std::string>()->default_value(""))(
      "l,level", "Iso surface value",
      cxxopts::value<float>()->default_value("0.15"))(
      "g,grid", "Grid size", cxxopts::value<unsigned>()->default_value("64"))(
      "b,builder", "Builder name [ref, loop, tree]",
      cxxopts::value<std::string>()->default_value("ref"))(
      "t,threads", "Number of OpenMP threads",
      cxxopts::value<unsigned>()->default_value("0"))(
      "batch", "Run in silent/batch mode")("h,help", "Print help");
  ...
  try {
    ...
    ParametricScalarField field(args["input"].as<std::string>(),
                                args["level"].as<float>());
    ...
    if (args["builder"].as<std::string>() == "ref") {
      ...
      std::cerr << "Unknown builder (" << args["builder"].as<std::string>()
                << ") specified!" << std::endl;
      std::exit(1);
    }
