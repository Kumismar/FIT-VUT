...
/** \brief findRelX najde retezec v definicnim oboru relace a pokud ano, vrati ukazetel na dany
 * prvek relace
 *
...
/** \brief findRelY najde retezec v oboru hodnot relace a pokud ano, vrati ukazetel na dany prvek
 * relace
 *
 * \param rel je ukazatel na relacni seznam
...
/** \brief findRelXY najde prvek s retezci v definicnim oboru i oboru hodnot relace a pokud ano,
 * vrati ukazetel na dany prvek relace oba prvky musi byt shodne
 *
 * \param rel je ukazatel na relacni seznam
 * \param x je ukazatel na retezec, ktery se vyhledava v definicnim oboru relace
 * \param y je ukazatel na retezec, ktery se vyhledava v oboru hodnot relace
...
/** \brief countRelXSets spocita pocet prvku, ktere maji retezec v definicnim oboru relace a jsou
 * mezi mnozinami set1 a set2
 *
 * \param rel je ukazatel na relacni seznam
 * \param x je ukazatel na retezec, ktery se vyhledava v definicnim oboru relace
...
int countRelXSets(TRelationItem* rel, TWordListItem* set1, TWordListItem* set2, char* x)
{
    int cnt = 0;
    while (rel != NULL) {
        if (strcmp(rel->name1, x) == 0 && strInSet(set1, rel->name1) == 1 &&
            strInSet(set2, rel->name2) == 1) {
            cnt++;
        }
        rel = rel->next;
    }
...
/** \brief countRelYSets spocita pocet prvku, ktere maji retezec v oboru hodnot relace a jsou mezi
 * mnozinami set1 a set2
 *
 * \param rel je ukazatel na relacni seznam
 * \param y je ukazatel na retezec, ktery se vyhledava v oboru hodnot relace
 * \param set1 je ukazatel na mnozinu s definicnim oborem
...
int countRelYSets(TRelationItem* rel, TWordListItem* set1, TWordListItem* set2, char* y)
{
    int cnt = 0;
    while (rel != NULL) {
        if (strcmp(rel->name2, y) == 0 && strInSet(set1, rel->name1) == 1 &&
            strInSet(set2, rel->name2) == 1) {
            cnt++;
        }
        rel = rel->next;
...
/** \brief cmdEmpty tiskne a vraci true nebo false podle toho, jestli je mnozina definovana na radku
 * A prazdna nebo neprazdna
 *
 * \param set1 je ukazatel na mnozinu
 ...
 *
 * \param set1 je ukazatel na mnozinu (radek), obsahuje ukazatel na sebe sama, dokud neni NULL, tak
 * se pocitaji prvky
 *
 */
...
/** \brief cmdSubseteqNoPrint vraci true nebo false podle toho, jestli je mnozina set1 podmnozinou
 * mnoziny set2
 *
...
/** \brief cmdSubseteq vraci a tiskne true nebo false podle toho, jestli je mnozina set1 podmnozinou
 * mnoziny set2
 *
 * \param set1 je ukazatel na mnozinu 1
 * \param set2 je ukazatel na mnozinu 2
...
int cmdSymmetric(TRelationItem* rel)
{
    ...
    while (tmpRel != NULL) {
        if (strcmp(tmpRel->name1, tmpRel->name2) != 0) {
            ...
            while (secRel != NULL) {
                if ((strcmp(tmpRel->name1, secRel->name2) == 0) &&
                    (strcmp(tmpRel->name2, secRel->name1) == 0)) {
                    break;
                }
                secRel = secRel->next;
            }
        ...
        if (strcmp(rel->name1, rel->name2) != 0) {
            ...
            while (tmpRel != NULL) {
                if ((strcmp(rel->name1, tmpRel->name2) == 0) &&
                    (strcmp(rel->name2, tmpRel->name1) == 0)) {
                    printf("false\n");
                    return false;
                }
                tmpRel = tmpRel->next;
...
/** \brief freeWordList uvolni pamet z celeho seznamu retezcu, vcetne retezcu o kterych predpoklada,
 * ze jsou alokovane dynamicky
 *
...
/** \brief freeRelationList uvolni pamet z celeho seznamu TRelationItem, vcetne retezcu o kterych
 * predpoklada, ze jsou alokovane dynamicky
 *
 * \param item je ukazatel na prvni prvek seznamu
 *
 */
...
/** \brief hasOnlyEnLetters zkontroluje, jestli retezec obsahuje pouze mala a velka pismena anglicke
 * abecedy
 *
...
/** \brief strInKeywords zjisti, jesli se retezec str vyskytuje mezi klicovymi slovy "true" a
 * "false"
 *
 * \param str je hledany retezec
...
/** \brief strInSetCmds1 zjisti, jesli se retezec str vyskytuje mezi mnozinovymi prikazy s 1
 * operandem
 *
 * \param str je hledany retezec
...
/** \brief strInSetCmds2 zjisti, jesli se retezec str vyskytuje mezi mnozinovymi prikazy se 2
 * operandy
 *
 * \param str je hledany retezec
 * \return vraci 1, kdyz je retezec mezi prikazy nalezen, jinak 0
...
/** \brief strInRelCmds1 zjisti, jesli se retezec str vyskytuje mezi relacnimi prikazy s jednim
 * operandem
 *
 * \param str je hledany retezec
 * \return vraci 1, kdyz je retezec mezi prikazy nalezen, jinak 0
...
{
    char* relCmds1[] = {
        "reflexive", "symmetric", "antisymmetric", "transitive",  "function",
...
/** \brief strInRelCmds3 zjisti, jesli se retezec str vyskytuje mezi relacnimi prikazy se tremi
 * operandy
 *
 * \param str je hledany retezec
 * \return vraci 1, kdyz je retezec mezi prikazy nalezen, jinak 0
...
/** \brief strInSetCreateCmds zjisti, jesli se retezec str vyskytuje mezi prikazy, ktere vytvareji
 * mnozinu
 *
 * \param str je hledany retezec
 * \return vraci 1, kdyz je retezec mezi prikazy nalezen, jinak 0
...
/** \brief strInRelationCreateCmds zjisti, jesli se retezec str vyskytuje mezi prikazy, ktere
 * vytvareji relaci
 *
 * \param str je hledany retezec
 * \return vraci 1, kdyz je retezec mezi prikazy nalezen, jinak 0
...
void printAllLines(TLine* line, int lineNr)
{
    for (int i = 0; i < lineNr; i++) {
        ...
        if (line[i].command != NULL) {
            printf("%4d: ", i + 1);
            if (strInSetCmds1(line[i].command->name) == 1 ||
                strInRelCmds1(line[i].command->name) == 1) {
 ...
 *  prikazy jsou pouze ulozeny, ale neprovedeny, protoze nemame zatim nactene vsechny radky a nevime
 * tedy, zda existuji
 *
...
int processLine(TWordListItem* token, int lineNr, int* wasCommand, TLine* line)
{
    ...
    switch (*(token->name)) {
        case 'U':
            ...
            while (token != NULL) {
                if (hasOnlyEnLetters(token->name) == 0) {
                    fprintf(stderr,
                            "ERROR: Vadny znak v prvku na radku %d. Prvky musi obsahovat jen mala "
                            "a velka pismena anglicke abecedy!\n",
                            lineNr);
                    return PL_ERR_INVALID_CHAR;
                }
                if (strInSetCmds1(token->name) == 1 || strInSetCmds2(token->name) == 1 ||
                    strInRelCmds1(token->name) == 1 || strInRelCmds3(token->name) == 1 ||
                    strInKeywords(token->name) == 1) {
                    fprintf(stderr,
                            "ERROR: Prvek na radku %d je obsazeny v prikazech nebo klicovych "
                            "slovech!\n",
                            lineNr);
        ...
        case 'S':
            ...
            if (line[0].set == NULL) {
                fprintf(stderr,
                        "ERROR: Univerzum nebylo definovano a mame mnozinu na radku %d!\n",
                        lineNr);
                return PL_ERR_UNIVERSE_NOT_LINE1;
            }
            if (*wasCommand == 1) {
            ...
            token = token->next;
            while (token != NULL) {
                /* pismena anglicke abecedy se nemusi kontrolovat u mnoziny a relace, protoze by
                jinak stejne nemohlo byt v univerzu, kde se to kontroluje
                if(hasOnlyEnLetters(token->name) == 0)
                {
                    fprintf(stderr, "ERROR: Vadny znak v prvku. Prvky musi obsahovat jen mala a
                velka pismena anglicke abecedy!\n"); return PL_ERR_INVALID_CHAR;
                } */
                if (strInSet(line[0].set, token->name) == 0) {
                    fprintf(stderr,
                            "ERROR: Prvek na radku %d neni obsazeny v Univerzu (vadne znaky?)!\n",
                            lineNr);
        ...
        case 'R':
            if (line[0].set == NULL) {
                fprintf(stderr,
                        "ERROR: Univerzum nebylo definovano a mame relaci na radku %d!\n",
                        lineNr);
                return PL_ERR_UNIVERSE_NOT_LINE1;
            }
            if (*wasCommand == 1) {
            ...
            while (token != NULL) {
                ...
                if (strInSet(line[0].set, name1) == 0 || strInSet(line[0].set, name2) == 0) {
                    fprintf(
                        stderr,
                        "ERROR: Prvek relace z radku %d neni obsazen v univerzu (vadne znaky?)!\n",
                        lineNr);
                    return PL_ERR_ITEM_NOT_IN_UNIVERSE;
                }
            ...
            *wasCommand = 1;
            ...
            while (token != NULL) {
                switch (param) {
                    ...
                    case 4:
                        for (size_t i = 0; i < strlen(token->name); i++) {
                            if (token->name[i] < '0' || token->name[i] > '9') {
                                fprintf(stderr,
                                        "ERROR: Parametr prikazu na radku %d neni cislo!\n",
                                        lineNr);
                        ...
                        if (op[param - 1] < 1 || op[param - 1] > MAX_LINES) {
                            fprintf(stderr,
                                    "ERROR: Parametr prikazu (radek) na radku %d je mimo rozsah "
                                    "1-1000!\n",
                                    lineNr);
                            return PL_ERR_BAD_CMD_PARAM;
                        }
                    ...
                    default:
                        fprintf(stderr,
                                "ERROR: Prikaz na radku %d ma vice nez 4 parametry!\n",
                                lineNr);
                        return PL_ERR_INVALID_CMD;
                }
                param++;
            ...
            // printf("Command: %s %d %d\n", line[lineNr-1].command->name,
            // line[lineNr-1].command->op1, line[lineNr-1].command->op2);
        ...
        default:
            fprintf(stderr,
                    "ERROR: Spatny prikaz na radku %d! Povolene prikazy jsou pouze U, S, R, C!\n",
                    lineNr);
            return PL_ERR_INVALID_CMD;
    }
    return 0;
 ...
 * \return funkce nic nevraci, ale v ch bude pri navratu prvni nebily znak na ktery narazi a tento
 * znak bude uz nacteny ze souboru
 *
 */
...
TWordListItem* getLineFromFile(FILE* fp)
{
    ...
    while (ch != EOF) {
        if (ch == ' ' || ch == '\t' || ch == '\n') {
            if (i == 0) {
                // mame konec radku, asi diky mezeram na konci, jinak jsme se sem dostat nemohli,
                // tak prazdny retezec uz pridavat nebudeme
            ...
            // jestli duvod konce tokenu byl konec radku, tak koncime, jinak preskocit mezery a
            // vynulovat pocitadlo znaku, bude asi dalsi token
            if (ch == '\n') {
                return tokenList;
            }
            else {
        ...
        if (line[ln].command != NULL) {
            ...
            if (operand2 == 0 || operand2 > lines) {
                if (strInSetCmds2(command) == 1 ||
                    strInRelCmds3(command) ==
                        1)  // druhy operand nas zajima jen pokud mame prikaz pro 2 operandy
                {
                    fprintf(stderr, STR_WRONG_PARAM_LINE, ln + 1);
                    return ERR_CMD_LINE_NOEX;
                }
            ...
            if (operand3 == 0 || operand3 > lines) {
                if (strInRelCmds3(command) ==
                    1)  // treti operand nas zajima jen pokud mame prikaz pro 3 operandy
                {
                    fprintf(stderr, STR_WRONG_PARAM_LINE, ln + 1);
                    return ERR_CMD_LINE_NOEX;
                }
            ...
            if (strInSetCmds1(command) == 1) {
                if (line[operand1 - 1].content != 'S') {
                    fprintf(stderr,
                            "ERROR: Prikaz pro mnozinu na radku %d ukazuje na radek, na kterem "
                            "neni mnozina!\n",
                            ln + 1);
            ...
            if (strInRelCmds1(command) == 1) {
                if (line[operand1 - 1].content != 'R') {
                    fprintf(stderr,
                            "ERROR: Prikaz pro relaci na radku %d ukazuje na radek, na kterem neni "
                            "relace!\n",
                            ln + 1);
                    return ERR_WRONG_LINE_TYPE;
                }
            ...
            if (strInRelCmds3(command) == 1 &&
                (line[operand1 - 1].content != 'R' || line[operand2 - 1].content != 'S' ||
                 line[operand3 - 1].content != 'S')) {
                fprintf(stderr,
                        "ERROR: Prikaz pro relaci na radku %d ukazuje na radek, na kterem neni "
                        "relace!\n",
                        ln + 1);
            ...
            // Relation commands
                ...
                if (!cmdInjective(line[operand1 - 1].relation,
                                  line[operand2 - 1].set,
                                  line[operand3 - 1].set)) {
                    JUMP(operand4);
                }
                ...
                if (!cmdSurjective(line[operand1 - 1].relation,
                                   line[operand2 - 1].set,
                                   line[operand3 - 1].set)) {
                    JUMP(operand4);
                }
                ...
                if (!cmdBijective(line[operand1 - 1].relation,
                                  line[operand2 - 1].set,
                                  line[operand3 - 1].set)) {
                    JUMP(operand4);
                }
                ...
                cmdClosureRef(line[operand1 - 1].relation,
                              line[UNIVERSUM_LINE - 1].set,
                              &line[ln].relation);
            }
...
int processFile(char* fileName)
{
    ...
    if (fp != NULL) {
        ...
        // definujeme a vytvorime vynulovane pole s informacemi pro max 1000 (MAX_LINES) radku
        // vstupniho souboru
        TLine* line;
        ...
        do {
            ...
            if (token != NULL) {
                ...
                lineNr++;
                // pokud mame vice radku nez na kolik je naalokovane pole radku, tak naalokovat
                // dalsi sadu radu
                if (lineNr > allocatedLines) {
                    ...
                    if (line == NULL) {
                        fprintf(stderr,
                                "ERROR: Nepodarilo se alokovat dostatek pameti pri rozsirovani "
                                "pole radku!\n");
                        exit(ERR_MALLOC);
                    }
                    // vynulovat nove naalokovanou oblast
                    memset(line + allocatedLines - LINES_ALLOC_SIZE,
                           0,
                           sizeof(TLine) * LINES_ALLOC_SIZE);
                }
...
#ifndef TEST_SET_SIZE
#    define TEST_SET_SIZE 3
#endif  // TEST_SET_SIZE
...
void generate_nondet_set(TWordListItem** head, unsigned size)
{
    *head = NULL;
...
/* Generuje nahodnou relaci */
void generate_nondet_relation(TRelationItem** head, unsigned size)
{
    *head = NULL;
    for (unsigned i = 0; i < size; i++) {
        char s1[STR_LEN + 1];
...
{
    // =================================================================
    // TEST CASE 1: Set Union (Correct)
    // =================================================================
#ifdef TEST_CASE_UNION
    TWordListItem* setA = NULL;
    TWordListItem* setB = NULL;
    TWordListItem* result = NULL;

    generate_nondet_set(&setA, TEST_SET_SIZE);
    generate_nondet_set(&setB, TEST_SET_SIZE);

    if (setA == NULL)
        return 0;

    // Svědek: Prvek z A
    char witness[STR_LEN + 1];
    strcpy(witness, setA->name);

    cmdUnion(setA, setB, &result);

    // Vlastnost 1: Prvek z A musi byt ve vysledku
    assert(strInSet(result, witness) == 1);

    // Vlastnost 2: Velikost vysledku >= velikost A
    assert(countElements(result) >= countElements(setA));
    ...
    // =================================================================
    // TEST CASE 2: Set Intersection (Průnik)
    // =================================================================
#ifdef TEST_CASE_INTERSECT
    TWordListItem* setA = NULL;
    TWordListItem* setB = NULL;
    TWordListItem* result = NULL;
    ...
    cmdIntersect(setA, setB, &result);

    // Pokud je vysledek neprazdny, vybereme z nej svedka
    if (result != NULL) {
        char witness[STR_LEN + 1];
        strcpy(witness, result->name);

        // Vlastnost: Prvek ve vysledku (Pruniku) MUSI byt v A a MUSI byt v B
        assert(strInSet(setA, witness) == 1);
        assert(strInSet(setB, witness) == 1);
    }

    // Vlastnost: Velikost pruniku nesmi byt vetsi nez velikost menši z mnozin
    int cntA = countElements(setA);
    int cntRes = countElements(result);
    assert(cntRes <= cntA);
    ...
    // =================================================================
    // TEST CASE 3: Symmetric Closure (Symetrický uzávěr)
    // =================================================================
#ifdef TEST_CASE_CLOSURE_SYM
    TRelationItem* rel = NULL;
    TRelationItem* result = NULL;

    generate_nondet_relation(&rel, TEST_SET_SIZE);

    cmdClosureSym(rel, &result);

    // Pokud puvodni relace nebyla prazdna, zkontrolujeme vlastnosti
    if (rel != NULL) {
        // Vlastnost 1: Puvodni dvojice (x, y) musi byt ve vysledku
        char x[STR_LEN + 1];
        char y[STR_LEN + 1];
        strcpy(x, rel->name1);
        strcpy(y, rel->name2);

        assert(findRelXY(result, x, y) != NULL);

        // Vlastnost 2: Ve vysledku musi byt i symetricka dvojice (y, x)
        assert(findRelXY(result, y, x) != NULL);
    }

    // Vlastnost 3: Výsledná relace musí být symetrická jako celek
    // (tzn. pro kazde (u, v) ve vysledku existuje (v, u))
    if (result != NULL) {
        char u[STR_LEN + 1];
        char v[STR_LEN + 1];
        strcpy(u, result->name1);
        strcpy(v, result->name2);

        // Pokud existuje (u, v), musi existovat (v, u)
        assert(findRelXY(result, v, u) != NULL);
    }

    freeRelationList(rel);
    freeRelationList(result);
#endif

    // =================================================================
    // TEST CASE 4: Relation Domain
    // =================================================================
#ifdef TEST_CASE_DOMAIN
    TRelationItem* rel = NULL;
    TWordListItem* domainSet = NULL;
    ...
    // Witness: x z prvni dvojice (x, y)
    char witness[STR_LEN + 1];
    strcpy(witness, rel->name1);

    cmdDomain(rel, &domainSet);

    // Assert: x musi byt v domene
    assert(strInSet(domainSet, witness) == 1);
    assert(domainSet != NULL);

    freeRelationList(rel);
    freeWordList(domainSet);
#endif
